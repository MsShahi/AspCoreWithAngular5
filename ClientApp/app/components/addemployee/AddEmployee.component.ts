...
export class createemployee implements OnInit {
    ...
    constructor(private _fb: FormBuilder, private _avRoute: ActivatedRoute, private _employeeService: EmployeeService, private _route: Router) {
    ...
    save() {
        ...
        if (this.title == "Create") {
            this._employeeService.saveEmployee(this.employeeForm.value)
                .subscribe((data) => {
                    this._route.navigate(['./fetch-employee']);
                }, error => this.errorMessage = error)
        }
        else if (this.title == "Edit") {
            this._employeeService.updateEmployeee(this.employeeForm.value)
                .subscribe((data) => {
                    this._route.navigate(['./fetch-employee']);
                }, error => this.errorMessage = error)
        }
    ...
    //this getter function is for client side validation
